...
def current_costs(public):
    ...
    try:
        headroom = str(max(0, amount(resources.money) + amount(resources.credit_limit)))
    except InvalidAction:
        headroom = None
    result = {
        "version": "public_costs_v2",
        "observation_id": observation.observation_id,
        "currency": "game_dollars",
        "cash_balance": resources.money,
        "credit_limit": resources.credit_limit,
        "positive_price_spending_headroom": headroom,
        "inventory": {
            ...
            offers.append(
                {
                    ...
                    "affordable": affordability(offer.price, resources),
                    "operation": purchase,
    ...
    if "sell" in available or purchase:
        result["owned_items"] = [
            ...
            for kind, cards in (("joker", state.jokers), ("consumable", state.consumables))
            for card in cards
