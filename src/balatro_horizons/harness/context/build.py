...
def context_bound(context, exchanges):
    """Measure both provider serializations plus fixed settings/framing padding."""
...
def _assembled_context(
    ...
    if frozen is None or frozen.get("memory_policy", {}).get("action_outcome") == VERSION:
        context.previous_action_outcome = previous_action_outcome(original, history)
    return context
...
def build_context(
    ...
    """Build in source order, then enforce the bounded delivery window."""
    ...
    context = _assembled_context(original, prompt, kernel, tools, costs, focused, omitted,
                                 notes, history, allowed, helper_status, frozen)
