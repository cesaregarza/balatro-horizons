"""Explicit, replaceable Windows connection; never inherit a service's stale session.

Registration runs in a connected WSL shell. Readers load it before each new
bridge process, so reconnecting neither restarts the backend nor replays actions.
"""
...
WINDOWS_VARIABLES = {
    ...
    "LOCALAPPDATA": "p", "PROGRAMDATA": "p", "COMSPEC": "p", "USERNAME": "", "USERDOMAIN": "",
}
SESSION_VARIABLES = ("WSL_INTEROP", "WSL_DISTRO_NAME", "WSL2_GUI_APPS_ENABLED")
RECONNECT = "Run scripts/configure_workbench_session.py --apply from a Windows-connected WSL terminal."
...
def session_environment(source):
    ...
    result = {key: source[key] for key in (*WINDOWS_VARIABLES, *SESSION_VARIABLES) if source.get(key)}
    if any(not isinstance(value, str) or any(c in value for c in "\n\r\x00") for value in result.values()):
        raise ValueError("INVALID_SESSION_ENVIRONMENT")
    ...
    result["WSLENV"] = ":".join(
        key + ("/" + flag if flag else "") for key, flag in WINDOWS_VARIABLES.items() if key in result
    )
    return result
...
def bridge_environment():
    ...
    return {**{key: os.environ[key] for key in ("PATH", "LANG", "LC_ALL") if key in os.environ}, **load_session()}
...
def connection_status():
    try:
        load_session()
        return {"ready": True, "code": None, "message": "Windows connection registered; game startup is verified when a run starts."}
    except ValueError as error:
        return {"ready": False, "code": str(error), "message": "Windows runtime connection needs refreshing. " + RECONNECT}
