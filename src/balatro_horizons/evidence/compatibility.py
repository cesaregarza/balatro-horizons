...
def _identity(sources):
    return {"native_hash": digest(native_component_manifest(sources)),
            "execution_hash": digest(execution_manifest(sources))}


def prepare_compatibility(source_hashes, protocol):
    ...
    if not previous:
        return None
    identity = _identity(source_files(ROOT))
    revisions = {}
    for source in sorted(previous):
        ...
        if fingerprint_sources(historical) != source or _identity(historical) != identity:
            raise ValueError("RESTORE_SOURCE_INCOMPATIBLE")
        revisions[source] = revision
    return {"version": "restore-source-v1", "accepted_implementation_hash": current,
