...
def prepare(root, candidate, manifest, *, allow_unrelated_changes=False):
    ...
    removed = git(root, "diff", "--no-renames", "--name-only", "--diff-filter=D", "HEAD", revision)
    if removed:
    ...
    for name in sorted(filter(None, names.split("\0"))):
        ...
        rows.append({"path": name, "before_sha256": digest(target), "after_sha256": digest(source)})
    if not rows:
...
def install(root, candidate, manifest, backup, *, snapshot_only=False):
    ...
    _verify_install_sources(root, candidate, rows)
    ...
    _write_backup_files(root, backup, files)
    ...
    if snapshot_only:
        ...
        return
    _install_files(root, candidate, rows)
...
def _verify_install_sources(root, candidate, rows):
    """Verify all source and destination checksums before installation writes."""
    for row in rows:
...
def _snapshot_paths(root):
    ...
    snapshots += [
        root / "private/capability-certificate.json",
        root / "data/operator-settings.json",
        root / "private/environment.lock.json",
        root / "private/rules.json",
    ]
    snapshots += [
        p
        for p in (root / "data/private_runs").glob("*/certificate-*.json")
        if re.fullmatch(r"certificate-\d+\.json", p.name)
    ]
    return snapshots


def _write_backup_files(root, backup, files):
...
def _install_files(root, candidate, rows):
    for row in rows:
...
def configure_parser(parser):
    ...
    parser.add_argument(
        "--prepare", action="store_true", help="Write a new checksum manifest from clean tracked checkouts; no install"
    )
    parser.add_argument(
        "--allow-unrelated-changes", action="store_true", help="With --prepare, permit live edits only outside the candidate file set"
    )
    parser.add_argument(
        "--snapshot-only", action="store_true", help="Validate the manifest and back up Linux files; do not install"
    )
    parser.add_argument(
        "--snapshot-runtime", action="store_true", help="Back up owned Windows instrumentation to Linux --backup; no install or launch"
    )
...
def run(args):
    ...
    if args.prepare:
        ...
        print(
            json.dumps(
                prepare(root, args.candidate, args.manifest, allow_unrelated_changes=args.allow_unrelated_changes),
                sort_keys=True,
            )
        )
        return None
    ...
    install(root, native(args.candidate), native(args.manifest), backup, snapshot_only=args.snapshot_only)
    return None
