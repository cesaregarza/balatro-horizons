...
def configure_parser(parser):
    ...
    mode.add_argument("--status", action="store_true", help="Show progress; records review exposure")
    parser.add_argument("--episode-id", help="Status for a specific episode; defaults to latest smoke")
...
def _episode_id(store, episode_id, campaign):
...
def _status_data(store, episode_id):
...
def _status_result(episode_id, manifest, events, summary):
    responses = [event for event in events if event["type"] == "provider_response"]
    settled = {event["request_id"]: event["payload"]["cost_usd"] for event in responses}
    pending = sum(
        event["payload"]["reserved_usd"]
        for event in events
        if event["type"] == "provider_request" and event["request_id"] not in settled
    )
    last = next((event["payload"] for event in reversed(events) if event["type"] == "observation"), {})
    errors = [
        event["payload"] for event in events if event["type"] in ("provider_error", "action_rejected")
    ]
    return {
        ...
        "committed_actions": sum(event["type"] == "action_commit" for event in events),
        "provider_calls": sum(event["type"] == "provider_request" for event in events),
        "responses_received": len(responses),
        "helper_calls_by_kind": dict(
            Counter(
                event["payload"]["operation"]["kind"]
                for event in events
                if event["type"] == "helper_result"
            )
        ),
        ...
        "last_error": {key: errors[-1].get(key) for key in ("code", "provider_code")}
        if errors
        else None,
    }
...
def episode_status(store, episode_id=None, *, campaign=None):
    ...
    episode_id = _episode_id(store, episode_id, campaign)
    manifest, events, summary = _status_data(store, episode_id)
    result = _status_result(episode_id, manifest, events, summary)
...
def _load_smoke_config(args):
    ...
    return config, limits, model


def _read_credential(args):
    if args.env_file:
...
def _preflight(args, config, limits, model):
    ...
    return campaign, ledger, blockers, check


def _freeze_campaign(campaign, args, config):
    frozen = (
...
def _execute_campaign(args, config, limits, ledger):
    ...
    return store, run_config, summary


def _write_result(store, run_config, summary):
    events = store.events(summary["episode_id"])
    responses = [event["payload"]["body"] for event in events if event["type"] == "provider_response"]
    result = {
        "summary": summary,
        "config_hash": digest(run_config.model_dump()),
        "responses_received": len(responses),
        "provider_errors": [
            {"code": event["payload"]["code"], "provider_code": event["payload"].get("provider_code")}
            for event in events
...
def _run_paid(args, config, limits, ledger, campaign):
    ...
    with (campaign / "campaign.lock").open("a") as guard:
        ...
        _freeze_campaign(campaign, args, config)
        store, run_config, summary = _execute_campaign(args, config, limits, ledger)
        result = _write_result(store, run_config, summary)
...
def run(args):
    ...
    config, limits, model = _load_smoke_config(args)
    campaign, ledger, blockers, check = _preflight(args, config, limits, model)
    print(json.dumps(check, indent=2), flush=True)
    if blockers or not args.allow_paid:
        return 1 if blockers else 0
    return _run_paid(args, config, limits, ledger, campaign)
