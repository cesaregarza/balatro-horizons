...
test("Sol and Astra expose their supported reasoning choices", () => {
  ...
  expect(() => configureModel(astra, "none", "tools_v6")).toThrow();
  expect(
    configureModel(astra, "max", "tools_v6").settings.reasoning_effort,
  ).toBe("max");
  expect(
    configureModel(astra, "max", "tools_v6").settings.harness_interface,
  ).toBe("tools_v6");
});
...
test("model defaults deduplicate aliases without rewriting historical settings", () => {
  ...
  const v6Alias = configureModel(terra, "medium", "tools_v6");
  const models = {
    "terra-v6": v6Alias,
    "terra-v5": v5Alias,
    "terra-tools": legacy,
  ...
  expect(modelCatalog(models)[modelKey(terra)]).toEqual(v6Alias);
  expect(
    modelCatalog({ "terra-v5": v5Alias, "terra-tools": legacy })[
      modelKey(terra)
    ],
  ).toEqual(v5Alias);
  expect(harnessLabel(v6Alias)).toBe("Current harness (v6)");
  expect(harnessLabel(v5Alias)).toBe("Legacy harness (tools_v5)");
  const selected = configureModel(terra, "high", "tools_v6");
  expect(
  ...
  expect(() => configureModel(terra, "minimal", "tools_v6")).toThrow();
  expect(() => configureModel(luna, "medium", "tools_v6")).toThrow();
  ...
  expect(configureModel(anthropic, "", "tools_v6").settings).toEqual({
    thinking_budget: 2048,
    harness_interface: "tools_v6",
  });
});
...
test("model, effort and harness persist; launching preserves fresh budgets and exact settings", async ({
  ...
  try {
    ...
    // A model still saved on v4 must also freeze new plans on v6.
