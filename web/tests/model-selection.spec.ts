...
test("Sol and Astra expose their supported reasoning choices", () => {
  ...
  expect(() => configureModel(astra, "none", "tools_v4")).toThrow();
  expect(
...
test("model defaults deduplicate aliases without rewriting historical settings", () => {
  ...
  const selected = configureModel(terra, "high", "operate_v1");
  expect(
...
test("model, effort and harness persist; launching preserves fresh budgets and exact settings", async ({
  ...
  try {
    ...
    await page
      ...
      .selectOption("high");
    await page.getByLabel("Harness", { exact: true }).selectOption("tools_v3");
    await page
    ...
    await page
      .getByLabel("Reasoning effort", { exact: true })
      .selectOption("max");
    await page.getByLabel("Harness", { exact: true }).selectOption("tools_v4");
