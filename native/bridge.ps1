param(
  [Parameter(Mandatory=$true)][ValidateSet('rpc','launch','stop')][string]$Mode,
...
while ($null -ne ($line = [Console]::ReadLine())) {
  try {
    ...
    $response = Invoke-WebRequest -Uri "http://127.0.0.1:$Port/" -Method Post -ContentType 'application/json' -Body ([System.Text.Encoding]::UTF8.GetBytes($body)) -TimeoutSec $TimeoutSec -UseBasicParsing
