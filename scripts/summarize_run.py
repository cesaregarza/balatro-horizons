...
def main():
    ...
    selection.add_argument("--episode-id")
    selection.add_argument("--latest-model", help="Newest episode for an exact provider model ID")
    parser.add_argument("--status-only", action="store_true", help="Compact verified status; no model text")
    ...
    if args.status_only and args.markdown_output:
        parser.error("--status-only cannot be combined with --markdown-output")
    try:
        eid = args.episode_id or latest_model_episode(store, args.latest_model)
    except ValueError as error:
        parser.error(str(error))
    result = run_status(store, eid) if args.status_only else build_summary(store, eid)
    markdown = None
    if args.markdown_output:
        from decision_summary import render_decisions

        markdown = render_decisions(result)
        scan({"markdown": markdown}, [store.manifest(eid, True).get("seed")])
