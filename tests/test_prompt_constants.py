...
def test_custom_prompt_bytes_cannot_bypass_template_validation(config):
    template = (ROOT / "configs/prompts/harness.txt").read_bytes()
    invalid = template.replace(b"{retained_helper_results}", b"7")
    with pytest.raises(HarnessFailure, match="PERSISTENT_INSTRUCTIONS_INVALID"):
        freeze_protocol(config, Baseline("heuristic"), {}, prompt_bytes=invalid)
