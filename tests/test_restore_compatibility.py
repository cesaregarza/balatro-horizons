...
@pytest.mark.parametrize("historical", [
    ...
    ("workbench/budget_continuation.py", b'if not increases_cap(binding_cap(old_cap, root_batch_cap), combined_cap):', b'if increases_cap(binding_cap(old_cap, root_batch_cap), combined_cap):'),
    ("cost_limits.py", b'requested > previous', b'requested >= previous'),
